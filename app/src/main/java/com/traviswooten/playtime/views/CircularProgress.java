...
import com.traviswooten.playtime.utils.Utils;
...
public class CircularProgress extends View {
    ...
    public CircularProgress(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        final TypedArray attributes = context.getTheme().obtainStyledAttributes(attrs, R.styleable.CircularProgress, defStyleAttr, 0);
    ...
    protected void initByAttributes(TypedArray attributes) {
        finishedStrokeColor = attributes.getColor(R.styleable.CircularProgress_circular_finished_color, default_finished_color);
        unfinishedStrokeColor = attributes.getColor(R.styleable.CircularProgress_circular_unfinished_color, default_unfinished_color);
        textColor = attributes.getColor(R.styleable.CircularProgress_circular_text_color, default_text_color);
        textSize = attributes.getDimension(R.styleable.CircularProgress_circular_text_size, default_text_size);
        startAngle = attributes.getInteger(R.styleable.CircularProgress_circular_start_angle, 0);

        setMax(attributes.getInt(R.styleable.CircularProgress_circular_max, default_max));
        setProgress(attributes.getInt(R.styleable.CircularProgress_circular_progress, 0));
        finishedStrokeWidth = attributes.getDimension(R.styleable.CircularProgress_circular_finished_stroke_width, default_stroke_width);
        unfinishedStrokeWidth = attributes.getDimension(R.styleable.CircularProgress_circular_unfinished_stroke_width, default_stroke_width);
        if (attributes.getString(R.styleable.CircularProgress_circular_prefix_text) != null) {
            prefixText = attributes.getString(R.styleable.CircularProgress_circular_prefix_text);
        }
        if (attributes.getString(R.styleable.CircularProgress_circular_suffix_text) != null) {
            suffixText = attributes.getString(R.styleable.CircularProgress_circular_suffix_text);
        }
        innerBackgroundColor = attributes.getColor(R.styleable.CircularProgress_circular_background_color, default_inner_background_color);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(centerText.equalsIgnoreCase("")) {
            centerText = prefixText + progress + suffixText;
        }
        ...
        canvas.drawArc(unfinishedOuterRect, getProgressAngle() - (360-startAngle), 360 - getProgressAngle(), false, unfinishedPaint);

        if (!TextUtils.isEmpty(centerText)) {
            float textHeight = textPaint.descent() + textPaint.ascent();
            canvas.drawText(centerText, (getWidth() - textPaint.measureText(centerText)) / 2.0f, (getWidth() - textHeight) / 2.0f, textPaint);
        }
